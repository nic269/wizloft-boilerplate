				...
				<CardContent className="space-y-3">
					<select
						...
						onChange={(event) => {
							...
							void loadInvitations(event.target.value);
						}}
					...
					{acceptUrl ? (
						<a className="block break-all text-sm font-medium text-primary hover:underline" href={acceptUrl}>
							{acceptUrl}
						</a>
					) : null}
						...
						{invitations.map((invitation) => (
							<li className="flex items-center justify-between gap-3 px-3 py-3" key={invitation.id}>
								<div className="min-w-0">
									...
									<p className="text-xs text-muted-foreground">{invitation.status.toLowerCase()}</p>
								</div>
								{invitation.status === "PENDING" ? (
									<Button onClick={() => void revoke(invitation.id)} size="sm" variant="destructive">
										Revoke
									</Button>
								) : null}
							</li>
