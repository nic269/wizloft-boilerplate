...
Provider packages expose status helpers with a `configured` flag. Missing optional credentials must not crash local
development. Local/dev fallbacks are allowed only when they are explicit and private: console mail, local object storage,
memory storage, and in-process jobs. S3/R2 storage remains private and uses server-side encryption for writes and signed
URLs.

## Alternatives Considered

1. Throw on missing optional provider env. Rejected because it breaks first-run local development.
